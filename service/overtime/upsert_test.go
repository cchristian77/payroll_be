...
func (suite *OvertimeServiceTestSuite) Test_Upsert() {
	...
	}{
		{
			...
			prepareMock: func() {
				...
				suite.repo.EXPECT().FindAttendanceByUserIDAndDate(ctx, gomock.Eq(authUser.ID), gomock.Any()).
					...
					Times(1)
				suite.repo.EXPECT().UpsertOvertime(ctx, gomock.Any()).Times(0)
			},
			wantErr:       true,
		...
		},
		{
			...
			prepareMock: func() {
				...
				suite.repo.EXPECT().FindAttendanceByUserIDAndDate(ctx, gomock.Eq(authUser.ID), gomock.Any()).
					Return(attendance, nil).
					Times(1)
				suite.repo.EXPECT().UpsertOvertime(ctx, gomock.Any()).
					Return(overtime, nil).
					Times(1)
			},
		},
	...
	for _, tc := range testCases {
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			// Act
			result, err := suite.overtimeService.Upsert(suite.ec, input)
